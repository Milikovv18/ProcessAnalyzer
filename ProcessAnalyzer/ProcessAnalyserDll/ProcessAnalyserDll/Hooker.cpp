...
void Hooker::previewHook(void* func2hook, uint8_t** previewBytes = nullptr, uint8_t** previewText = nullptr) {
    if (previewBytes == nullptr && previewText == nullptr) return;
    ...
    if (previewBytes != nullptr) {
        *previewBytes = new uint8_t[stolenInstrs.numBytes + 1];
        uint8_t* curBytes = *previewBytes;
        for (uint32_t i = 0; i < stolenInstrs.numInstructions; ++i) {
            memcpy(curBytes, stolenInstrs.instructions[i].bytes, stolenInstrs.instructions[i].size);
            curBytes += stolenInstrs.instructions[i].size;
        }
        *curBytes = '\0';
    }
    ...
    if (previewText != nullptr) {
        size_t previewLen = 0;
        for (uint32_t i = 0; i < stolenInstrs.numInstructions; ++i) {
            auto instr = stolenInstrs.instructions[i];
            previewLen += strlen(instr.mnemonic) + strlen(instr.op_str) + 2;
        }

        *previewText = new uint8_t[previewLen];

        uint8_t* curText = *previewText;
        for (uint32_t i = 0; i < stolenInstrs.numInstructions; ++i) {
            //std::cout << stolenInstrs.instructions[i].mnemonic << " " << stolenInstrs.instructions[i].op_str << "\n";
            auto instr = stolenInstrs.instructions[i];
            size_t mnemLen = strlen(instr.mnemonic),
                opLen = strlen(instr.op_str);

            memcpy(curText, instr.mnemonic, mnemLen);
            curText += mnemLen;
            *curText = ' ';
            memcpy(++curText, instr.op_str, opLen);
            curText += opLen;
            if (i == stolenInstrs.numInstructions - 1) *curText = '\0';
            else *curText = '\n';
            ++curText;
        }
    }
...
void Hooker::uninstallHook(void* func2hook, void** trampolinePtr, uint8_t* rsrvBytes, const int& byteLen) {
    memcpy(func2hook, rsrvBytes, byteLen);
    VirtualFree(*trampolinePtr, 0, MEM_RELEASE);
}